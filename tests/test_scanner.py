...
@pytest.mark.parametrize(
    "name",
    ["api.pb.go", "svc.pb.gw.go", "thing.gen.go", "x_pb2.py", "y_pb2_grpc.py", "m.g.dart"],
)
def test_generated_detected_by_filename(tmp_path: Path, name):
    target = tmp_path / name
    target.write_text("package main" + chr(10))
    assert looks_generated(target)
...
def test_generated_detected_by_content_marker(tmp_path: Path, header):
    """Tesla's repo matched entirely on filename, so this path needs its
    own coverage -- it is the language-agnostic signal."""
    target = tmp_path / "handwritten_name.go"
    target.write_text(header + chr(10) + "package main" + chr(10))
    assert looks_generated(target)
...
@pytest.mark.parametrize(
    "content",
    [
        "package main" + chr(10) + "// this code was generated in 2019 by a person",
        "# a comment about code generated elsewhere, 30 lines down"
        + (chr(10) * 30)
        + "# code generated by something",
        "def handler():" + chr(10) + "    return 1",
    ],
    ids=["prose mentioning generation", "marker below header window", "ordinary source"],
)
def test_handwritten_files_are_not_flagged_generated(tmp_path: Path, content):
    target = tmp_path / "app.py"
    target.write_text(content)
    assert not looks_generated(target)


def test_discover_splits_generated_from_handwritten(tmp_path: Path):
    (tmp_path / "app.go").write_text("package main" + chr(10))
    (tmp_path / "api.pb.go").write_text("package main" + chr(10))
    (tmp_path / "svc.go").write_text(
        "// Code generated by protoc. DO NOT EDIT." + chr(10) + "package main" + chr(10)
    )

    regular, generated = discover_files_detailed(tmp_path)
    assert [p.name for p in regular] == ["app.go"]
    assert sorted(p.name for p in generated) == ["api.pb.go", "svc.go"]


def test_generated_excluded_by_default_but_available_on_request(tmp_path: Path):
    (tmp_path / "app.go").write_text("package main" + chr(10))
    (tmp_path / "api.pb.go").write_text("package main" + chr(10))

    assert [p.name for p in discover_files(tmp_path)] == ["app.go"]
    assert sorted(p.name for p in discover_files(tmp_path, include_generated=True)) == [
        "api.pb.go",
        "app.go",
    ]


def test_explicitly_named_generated_file_is_honoured(tmp_path: Path):
    """Pointing at a file by name is an explicit request; silently
    scanning nothing would be more confusing than obeying."""
    target = tmp_path / "api.pb.go"
    target.write_text("package main" + chr(10))
    assert discover_files(target) == [target]


def test_scanner_skips_generated_and_reports_the_count(tmp_path: Path):
    (tmp_path / "app.go").write_text("package main" + chr(10))
    (tmp_path / "api.pb.go").write_text("package main" + chr(10))

    client = CountingClient()
    report = Scanner(client, concurrency=1).scan(tmp_path)

    assert client.calls == 1, "generated file must not cost an API request"
    assert report.summary.generated_files_skipped == 1
    assert [r.file_path for r in report.results] == ["app.go"]


def test_scanner_include_generated_scans_everything(tmp_path: Path):
    (tmp_path / "app.go").write_text("package main" + chr(10))
    (tmp_path / "api.pb.go").write_text("package main" + chr(10))

    client = CountingClient()
    report = Scanner(client, concurrency=1, include_generated=True).scan(tmp_path)

    assert client.calls == 2
    assert report.summary.generated_files_skipped == 0


def test_generated_count_survives_baseline_rebuild(tmp_path: Path):
    """rebuild_summary() is called again after baseline processing; it must
    not zero a count it was never told about."""
    (tmp_path / "app.go").write_text("package main" + chr(10))
    (tmp_path / "api.pb.go").write_text("package main" + chr(10))

    report = Scanner(CountingClient(), concurrency=1).scan(tmp_path)
    assert report.summary.generated_files_skipped == 1
