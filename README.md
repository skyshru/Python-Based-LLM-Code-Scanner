...
| **Generated-code detection** | Protobuf stubs and codegen output are skipped by default — no wasted quota, no noise |
...
Machine-generated files — protobuf stubs, gRPC gateways, codegen output — are excluded by default. Nobody hand-writes or fixes them, so findings there are noise, and on a metered API they are pure wasted spend.

Detection uses two signals: known filename patterns (`.pb.go`, `_pb2.py`, `.g.dart`, …) and the standard in-file banners (`Code generated ... DO NOT EDIT.`, `@generated`, `<auto-generated>`) that generators emit in the first few lines.

Measured on Tesla's [`vehicle-command`](https://github.com/teslamotors/vehicle-command) repo: **9 generated files accounted for 45 of 161 requests — 28% of the entire scan.** Skipping them cuts a full scan from ~8 days to ~6 on a free-tier key.

```bash
llm-appsec-scanner -t ./src                       # generated files skipped
llm-appsec-scanner -t ./src --include-generated   # scan them too
llm-appsec-scanner -t ./api.pb.go                 # explicit target is always honoured
```

Test files are *not* skipped, deliberately: test fixtures are a common place for real credentials to leak, so those findings are worth having.
