...
### 3c. Generated-code exclusion ✅

Prompted by a concrete planning question — *could the Tesla scan be finished in ~15 days on a free tier?* — measuring the real cost first turned up something more useful than the answer.

**Measured, not estimated.** Running the actual chunker over `teslamotors/vehicle-command`: 161 requests across 110 files. Breaking that down by category was the finding:

| Category | Files | Requests | Share |
| --- | --- | --- | --- |
| Generated protobuf (`.pb.go`) | 9 | **45** | 28% |
| Test files (`_test.go`) | 31 | 37 | 23% |
| Hand-written source | 71 | 79 | 49% |

**Over half the scan was going to code nobody would act on.** Generated protobuf especially: 8% of the files, 28% of the cost, and any finding in it is unfixable by definition — the next `protoc` run overwrites the fix.

- `looks_generated()` uses two signals: filename suffixes (`.pb.go`, `_pb2.py`, `.g.dart`, …) which need no file read, and the standard in-file banners (`Code generated ... DO NOT EDIT.`, `@generated`, `<auto-generated>`) that Go and GitHub's linguist both standardise. Only the first 20 lines are examined — deeper scanning starts matching prose *about* generated code.
- `discover_files_detailed()` returns `(handwritten, generated)` from one walk, so the count can be reported without a second traversal.
- **Explicitly named files are still honoured**: `--target ./api.pb.go` scans it. Refusing a file the user named by hand would be more confusing than obeying.
- **Test files deliberately kept**, despite being another 23%. Test fixtures leak real credentials often enough that those findings have value; generated code has none. Cost alone was not sufficient reason to skip them.

**Result on the motivating question:** the Tesla scan drops from 161 to **116 requests — ~8 days to ~6** on a free tier. Not the headline saving, but real, and it applies to every Go/gRPC/protobuf project rather than just this one.

**Bug caught during implementation.** `rebuild_summary()` is called a second time by the CLI after baseline processing. With a plain `= 0` default, that second call silently zeroed the skipped-files count. Changed to `None`-means-preserve, with a regression test — exactly the summary-drift class of bug the "one source of truth" design in Phase 1 was meant to prevent.
